...
{
    int             i, j, ndonor[MAXSEQLEN], nacceptor[MAXSEQLEN], nhbs=0;
    float           hb_energy = 0.0F, costheta, e;
    float           dho, dcn, dnca, dcca, dno, hvlen;
    Point           c_nvec, ca_nvec, n_hvec;
    ...
    for (i = 0; i < seqlen; i++)
	if (seq[i] != PRO && !ndonor[i])
	    for (j = 0; j < seqlen; j++)
		if (abs(j-i) > 2 && distmat[i][j][CA_CA] < 9.0F && nacceptor[j] < 2)
		{
			...
			e = 5.5F * (5.0F * powf(2.9F / dno, 12.0F) - 6.0F * powf(2.9F / dno, 10.0F)) * SQR(SQR(costheta));
		    }
		    else
		    {
...
float           torsion(Point a, Point b, Point c, Point d)
{
    int             i;
...
void
    maketemplate(char *pdbname, char chainid, int model, RESATM *chn, char *pdbTmpPath)
{
    FILE           *pfp, *dfp, *xfp;
    char            buf[160], whichatm[MAXSEQLEN], inscode, pdbPath[1024];
    ...
    sprintf(pdbPath, "%s%s", pdbTmpPath, pdbname);
...
int main(int argc, char **argv)
{
    ...
    if (argc < 4)
    {
	    fprintf(stderr, "usage : %s pdbfname modcheckpot.dat tmpPath [chainid]\n", argv[0]);
	    exit(1);
    }
...
//    ifp = fopen("/var/www/cgi-bin/psipred/modcheckpot.dat", "rb");
...
;
    ...
    if (argc != 3)
    {
	     maketemplate(argv[1], ' ', -1, bestchn, argv[3]);
    }
    else if (argc == 3)
    {
	    if (strlen(argv[4]) != 1)
	      ...
	      a = -1;
	    maketemplate(argv[1], argv[4][0], a, bestchn, argv[3]);
    }
    if (seqlen < 20)
    {
	    printf(" 999\n");
    ...
    printf(" %f %f %f %f %f %f %d %f %f %f\n", -natpair * RTCONST, zpair, -natsolv * RTCONST, zsolv, zpair+zsolv, ehb, calcdisu(bestchn, distmat), radgyr, contord, min_ncd);

//    printf(" %f\n", -(zpair+zsolv));
    printf(" %f\n", -(0.29*zpair+0.17*zsolv));
