    ...
    char          **argv;
{
    ...
    if (argc < 5)
	fail("usage : tmjury3d_mq_modeller target-seq mqapscore ensemble.pdb ntemplates {mintmsc}");
	...
	if (i == MAXNSTRUC)
	    exit(-1);
	if (fscanf(ifp, "%*s%f", &mqapscore[i]) != 1)
	    break;
//	printf("%d %d %f\n", i, j, mqapscore[i]);
	if (!isnan(mqapscore[i]) && !isinf(mqapscore[i]) && mqapscore[i] < 500.0)
	{
	    mqapok[j++] = 1;
	    i++;
	}
    ...
    ifp = fopen(argv[3], "r");	/* Open PDB file in TEXT mode */
    if (!ifp)
    {
...
//	printf("%d %d\n", i, natoms[nmodels]);
	...
	else
	{
	    ...
	    {
		printf("Warning - model %d has bad MQAP score!!\n", nmodels+1);
		free(atoms[nmodels]);
		atoms[nmodels] = NULL;
		natoms[nmodels] = 0;
	    }
	...
	printf("%d %f\n", moda+1, scoretab[moda].score);
    }
    
    qsort(scoretab, nmodels, sizeof(struct mscore), cmpsc);

#if 0
    if (nmodels > ntemplates*5)
    {
	/* Rescore top ntemplates*2 models with just MQAP score */
	for (moda=0; moda<MIN(nmodels, ntemplates*2); moda++)
	    scoretab[moda].score = -mqapscore[scoretab[moda].modelnum];

	qsort(scoretab, ntemplates*2, sizeof(struct mscore), cmpsc);
    }
#endif
    
    afp = fopen("jury.aln", "w");
    if (!afp)
	...
	if (!scoretab[i].skipflg)
		...
		if (incompatmat[scoretab[i].modelnum][scoretab[j].modelnum])
		{
	...
	seqscore(targstr, atoms[moda], 1, 0, &tplt, seqlen, natoms[moda]);
	...
	if (atoms[moda][0].chain == ' ')
	    ...
	    fprintf(afp, ">P1;0t%02d\nStructureX:0t%02d:FIRST:%c:LAST:%c::::\n", i+1, i+1, atoms[moda][0].chain, atoms[moda][0].chain);
	for (j = 1; j <= tplt.length; j++)
	    if (tplt.posn_b[j] > 0)
		fputc(rescodes[atoms[moda][tplt.posn_b[j]-1].aac], afp);
	    else
		fputc('-', afp);
	fprintf(afp, "*\n");
    }
    ...
    afp = fopen("jurymod.py", "w");
    if (!afp)
	fail("Cannot open Modeller script file!");
    fprintf(afp, "from modeller import *\nfrom modeller.automodel import *\nenv = environ()\nenv.io.atom_files_directory = ['.']\n");
    fprintf(afp, "a = automodel(env,\nalnfile  = 'jury.aln',\nknowns   = (");
    for (i=0; i<ntemplates-1; i++)
	fprintf(afp, "'0t%02d', ", i+1);
    fprintf(afp, "'0t%02d'),", ntemplates);
    fprintf(afp, " sequence = 'SEQ')\na.make()\n");
    fclose(afp);
