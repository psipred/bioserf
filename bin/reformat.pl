...
#
# updated by Michael Remmert (October 2006)
...
Options:
  ...
  -nobb     remove beta-barrel prediction sequences (beginning with >bb_)
  ...
  -cons     make a consensus sequence
...
my $nobb=0;                 # don't remove transmembrane beta-barrel prediction
my $cons=0;                   # don't make consensus sequence
my $line;
my $options="";
...
if ($options=~s/ -nobb / /)   {$nobb=1;}
if ($options=~s/ -cons / /)   {$cons=1;}
...
#		if ($nss>=0) {next;}  # comment out: allow for two or more >ss_dssp and >ss_pred lines
	    ...
	    if ($seq=~/^bb_/) {
		if ($nobb) {next;}    # do not read in >bb_ sequences
	    }
	...
	while ($line = <INFILE>)
	{
	    ...
	    if (!(exists $seqhash{$1})) 
	    {
		if ($noss && $line=~/^(aa|ss|sa)_/) {next;} # do not read in >ss_ and >aa_ sequences
		if ($nobb && $line=~/^bb_/) {next;} # do not read in >bb_ sequences
...
#	    printf("%3i %3i %-16.16s %s\n",$block,$k,$names[$k],$seqs[$k]);
   ...
   # Read psi format
	...
	while ($line = <INFILE>) #scan through PsiBlast-output line by line
	{
	    ...
	    if ($line=~/^(aa|ss|sa|bb)_/) {next;} # do not read in >ss_ and >aa_ and >bb_ sequences
	    $names[$n]=$1;
	    $seqs[$n]=$2;
	    $n++;
	}
...
################################################################################################
	...
	for ($k=0; $k<scalar(@names); $k++) {  #find seed sequence
	    if ($names[$k]!~/^(ss_|aa_|sa_|bb_)/) {last;}
	}
	@residues=unpack("C*",$seqs[$k]);
	for (my $l=0; $l<@residues; $l++) {
	...
	# Set columns with residue in first sequence to upper case,
	for ($k=0; $k<$n; $k++) {
	    @residues=unpack("C*",$seqs[$k]);
	    $residues="";
	    for (my $l=0; $l<@residues; $l++) {
...
########################################################
    ...
    if ($cons) {
	    ...
	    @residues=unpack("C*",$seqs[$k]);
	    for (my $l=0; $l<@residues; $l++) {
		if ($residues[$l] > 96) {                   # a-z
		    next;
		} elsif ($residues[$l] > 64) {              # A-Z
		    if (defined $cons_char[$l][$residues[$l]-65]) {$cons_char[$l][$residues[$l]-65]++;} else {$cons_char[$l][$residues[$l]-65]=1;}
		}
	    }
	}
	...
	for (my $l=0; $l<@residues; $l++) {
	    ...
	    my $char = ".";
	    for (my $m=0; $m < 26; $m++) {
		if (defined $cons_char[$l][$m] && $cons_char[$l][$m] > $max) {
		    $max = $cons_char[$l][$m];
		    $char = chr($m + 65)
		}
	    }
	...
	unshift(@names, "$tmp_name CONSENSUS sequence");
	unshift(@seqs, $cons_seq);
	$n++;
    }
...
################################################################################################
	...
	elsif ($names[$k]=~/^bb_pred/){$nbbpred=$k; }
	elsif ($names[$k]=~/^bb_conf/){$nbbconf=$k; }
	...
	if ($num) {
	    my $num=2;
	    for ($k=0; $k<$n; $k++) {
		if ($k==$ndssp || $k==$npred || $k==$nconf || $k==$nbbpred || $k==$nbbconf || $k==$nquery) {next;}
		$names[$k]=~s/^(\S+)\#\d+/$1/;
	...
	if ($outformat eq "sto") {
	    for ($k=0; $k<$n; $k++) {
		if ($k==$ndssp || $k==$npred || $k==$nconf || $k==$nbbpred || $k==$nbbconf) {next;}
		$names[$k] =~ /\s*(\S+)/;
		if (!$lname) {$lname=32;}
		printf(OUTFILE "%-$lname.$lname"."s %s\n",$1,$seqs[$k]);
	    }
	    ...
	    my @blocks;
	    for ($k=0; $k<((length($seqs[0])/($numres-30))+1); $k++) { $blocks[$k] = ""; }
	    for ($k=0; $k<$n; $k++) {
		if ($k==$ndssp || $k==$npred || $k==$nconf || $k==$nbbpred || $k==$nbbconf) {next;}
		$names[$k] =~ /\s*(\S+)/;
		my $tmp_name = $1;
		if (!$lname) {$lname=32;}
		my $counter = 0;
		for (my $i = 0; $i < length($seqs[$k]); $i += ($numres-30)) {
		    $blocks[$counter] .= sprintf("%-$lname.$lname"."s %s\n",$1,substr($seqs[$k],$i,($numres-30)));
		    $counter++;
		}
	    }
	    my $res = "";
